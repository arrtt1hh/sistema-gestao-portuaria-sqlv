...
-- [ITEM 1] CONSTRAINT EXPLÍCITA (Checklist: Constraints slide 4)
-- Garante que ninguém cadastre carga com peso zero ou negativo
ALTER TABLE cargas ADD CONSTRAINT chk_peso_positivo CHECK (peso_kg > 0);

-- [ITEM 5] PERFORMANCE (Checklist: Criar Index)
-- Índices para acelerar buscas nas colunas mais usadas
CREATE INDEX idx_viagem_manifesto ON viagens(manifesto_carga_doc);
CREATE INDEX idx_cargas_cliente ON cargas(cliente);

-- [ITEM 4] SEGUNDA CONSULTA DE AGREGAÇÃO (Checklist: Pelo menos 2 consultas agrupadas)

SELECT '--- [3] RELATÓRIO: CAPACIDADE TOTAL POR PAÍS (BANDEIRA) ---' as relatorio_info;
SELECT 
    n.bandeira,
    COUNT(n.id_navio) AS qtd_navios,
    SUM(n.capacidade_teu) AS capacidade_total_teus
FROM navios n
GROUP BY n.bandeira
ORDER BY SUM(n.capacidade_teu) DESC;

-- [ITEM 6] USUÁRIOS E PERMISSÕES (Checklist: 2 usuários com permissões)
-- Criação de roles para simular segurança

-- Usuário 1: Operador (Pode ver tudo e inserir atracação/log)
DROP USER IF EXISTS operador_patio;
CREATE USER operador_patio WITH PASSWORD 'operador123';
GRANT USAGE ON SCHEMA shipping_db TO operador_patio;
GRANT SELECT ON ALL TABLES IN SCHEMA shipping_db TO operador_patio;
GRANT INSERT, UPDATE ON atracacoes TO operador_patio;
GRANT INSERT ON logs_sistema TO operador_patio;

-- Usuário 2: Auditor (Só pode ver relatórios/views)
DROP USER IF EXISTS auditor_financeiro;
CREATE USER auditor_financeiro WITH PASSWORD 'audit123';
GRANT USAGE ON SCHEMA shipping_db TO auditor_financeiro;
GRANT SELECT ON vw_dashboard_operacional TO auditor_financeiro;
